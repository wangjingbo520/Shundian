package com.eims.shundian.common.widgets.search;
...
public class FairySearchView extends LinearLayout {
    ...
    private int backIcon = R.mipmap.ic_back;//返回按钮图标
    private int searchIcon = R.mipmap.ic_search_gray;//搜索按钮图标
    private int clearIcon = R.mipmap.ic_clear;//清除按钮图标
    private String cancelText = getResources().getString(R.string.btn_cancel_text);
    private int cancelTextSize = getResources().getDimensionPixelSize(R.dimen.textSize_32);
    private int cancelTextColor = getResources().getColor(R.color.btn_cancel_text);
    private String searchText = "";
    private int searchTextSize = getResources().getDimensionPixelSize(R.dimen.textSize_32);
    ...
    private int searchViewHeight = getResources().getDimensionPixelSize(R.dimen.textSize_16);

    private int maxSearchLength = -1;//输入内容最大长度（默认不设限制）
    private int searchPaddingLeft = getResources().getDimensionPixelSize(R.dimen.textSize_16);
    //输入框左侧内边距（在没有左侧搜索图标时使用）
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable
                    .FairySearchView);
            showBackButton = typedArray.getBoolean(R.styleable.FairySearchView_showBackButton,
                    false);
            showSearchIcon = typedArray.getBoolean(R.styleable.FairySearchView_showSearchIcon,
                    true);
            showCancelButton = typedArray.getBoolean(R.styleable
                    .FairySearchView_showCancelButton, true);
            showClearButton = typedArray.getBoolean(R.styleable.FairySearchView_showClearButton,
                    true);

            backIcon = typedArray.getResourceId(R.styleable.FairySearchView_backIcon, R.mipmap
                    .ic_back);
            searchIcon = typedArray.getResourceId(R.styleable.FairySearchView_searchIcon, R
                    .mipmap.ic_search_gray);
            clearIcon = typedArray.getResourceId(R.styleable.FairySearchView_clearIcon, R.mipmap
                    .ic_clear);
            cancelText = getOrDefault(typedArray.getString(R.styleable
                    .FairySearchView_cancelText), resources.getString(R.string.btn_cancel_text));
            cancelTextSize = typedArray.getDimensionPixelSize(R.styleable
                    .FairySearchView_cancelTextSize, resources.getDimensionPixelSize(R.dimen
                    .textSize_16));
            cancelTextColor = typedArray.getColor(R.styleable.FairySearchView_cancelTextColor,
                    resources.getColor(R.color.btn_cancel_text));
            searchText = typedArray.getString(R.styleable.FairySearchView_searchText);
            searchTextSize = typedArray.getDimensionPixelSize(R.styleable
                    .FairySearchView_searchTextSize, resources.getDimensionPixelSize(R.dimen
                    .textSize_16));
            searchTextColor = typedArray.getColor(R.styleable.FairySearchView_searchTextColor,
                    resources.getColor(R.color.edit_text));
            searchHint = getOrDefault(typedArray.getString(R.styleable
                    .FairySearchView_searchHint), resources.getString(R.string.edit_hint));
            searchHintColor = typedArray.getColor(R.styleable.FairySearchView_searchHintColor,
                    resources.getColor(R.color.edit_hint_text));
            searchViewHeight = typedArray.getDimensionPixelSize(R.styleable
                    .FairySearchView_searchViewHeight, resources.getDimensionPixelSize(R.dimen
                    .textSize_16));
            maxSearchLength = typedArray.getInteger(R.styleable.FairySearchView_maxSearchLength,
                    -1);//默认不限制
    ...
    private void limitSearchViewHeight(int searchViewHeight) {
        LinearLayout.LayoutParams layoutParams = (LayoutParams) searchView.getLayoutParams();
